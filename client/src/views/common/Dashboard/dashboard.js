...
const Dashboard = () => {
  ...
  useEffect(() => {
    setisUser((user ? (user.result.googleId ? user.result.googleId : user.result.id) : "null") === userProfile.profile.user ? true : false);
  }, [userProfile]);
    ...
    <div className="container-fluid dashboard py-5 ">
      <div className="row profile-container mx-3 py-3">
        ...
        <div className="col-12 col-md-7 col-sm-7 col-lg-7 mx-auto d-block ">
          <p className="  fs-1 fw-lighter">
            {userProfile.user.name}

            {isUser == true && (
              <span>
                {" "}
                <a href="/edit-profile" as={Link} className="btn btn-outline-success">
                  Edit Profile
                </a>
              </span>
            )}
